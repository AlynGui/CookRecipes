...
// Recipes Section 1 - Search
const searchInput = document.querySelector('#recipes-section1-searchInput') as HTMLInputElement;
const searchBtn = document.querySelector('#recipes-section1-searchBtn')!;
...
searchBtn.addEventListener('click', () => {
  ...
  const filteredData = recipeData.filter(item =>
    ...
    item.foodStyle.toLowerCase().includes(query)
  );
...
// Recipes Section 2 - Paginated List
const ITEMS_PER_PAGE = 8;
let currentPage = 1;

const listContainer = document.querySelector('#recipes-section2-list')!;
const pageNumbers = document.querySelector('#recipes-section2-pageNumbers')!;
const prevBtn = document.querySelector('#recipes-section2-prevBtn') as HTMLButtonElement;
const nextBtn = document.querySelector('#recipes-section2-nextBtn') as HTMLButtonElement;

function renderList(recipes: typeof recipeData) {
  ...
  recipes.slice(start, end).forEach(item => {
    const card = Card1(item);
    listContainer.appendChild(card);
  });
}

function renderPagination(recipes: typeof recipeData) {
  pageNumbers.innerHTML = '';

  if (recipes.length === 0) {
    prevBtn.style.display = 'none';
    nextBtn.style.display = 'none';
    listContainer.innerHTML = '<p id="no-recipes-found">No recipes found.</p>';
    return;
  }
...
nextBtn.addEventListener('click', () => {
  const totalPages = Math.ceil(recipeData.length / ITEMS_PER_PAGE);
...
function update() {
  ...
  renderPagination(recipeData);
}

update();


// Recipes Section 3 - Subscribe
const recipes_section3 = document.querySelector('#recipes-section3');
if (!recipes_section3) {
  throw new Error('Element with ID "recipes-section3" not found.');
}
recipes_section3.appendChild(Subscribe());
