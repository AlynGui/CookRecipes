...
const dishData = recipeData.slice(8, 12); // Exporting data for use in other modules

// Add Header to contact page
document.body.prepend(Header());

const contact_section1_container = document.querySelector('#contact-section1-container');
if (!contact_section1_container) {
  throw new Error('Element with ID "contact-section1-container" not found.');
}
contact_section1_container.appendChild(contactCard());

// Add Subscribe section to contact page
const contact_section2 = document.querySelector('#contact-section2');
if (!contact_section2) {
  throw new Error('Element with ID "contact-section2" not found.');
}
contact_section2.appendChild(Subscribe());

// Add Cards to contact page
const contact_section3_bottom = document.querySelector('#contact-section3-bottom');
if (!contact_section3_bottom) {
  throw new Error('Element with ID "contact-section3-bottom" not found.');
}

dishData.forEach(dish => {
  const card = Card1(dish);
  contact_section3_bottom.appendChild(card);
});
