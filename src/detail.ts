...
import { detailCard } from './components/detailCard.ts';
import { Card3 } from './components/card3.ts';
...
const detail_data = recipeData.filter(recipe => recipe.id.toString() == id)[0];

const other_recipe_data = recipeData.filter(recipe => recipe.id.toString() != id).slice(0, 3);

const dishData = recipeData.filter(recipe => recipe.id.toString() != id).slice(10, 14);

// Header
document.body.prepend(Header());

// Detail Section 1 - Detail Card
const detail_section1_container = document.querySelector('#detail-section1-container');
if (!detail_section1_container) {
  throw new Error('Element with ID "detail-section1-container" not found.');
}
const detail_card = detailCard(detail_data);
detail_section1_container.appendChild(detail_card);

// Detail Section 2 - Ingredients and Instructions
const detail_ingredients_list = document.querySelector('#detail-ingredients-list');
if (!detail_ingredients_list) {
  throw new Error('Element with ID "detail-ingredients-list" not found.');
}
detail_data.ingredients.forEach(ingredient => {
  const ingredient_li = document.createElement('li');
  ingredient_li.textContent = ingredient;
  detail_ingredients_list.appendChild(ingredient_li);
});

const detail_directions_list = document.querySelector('#detail-directions-list');
if (!detail_directions_list) {
  throw new Error('Element with ID "detail-directions-list" not found.');
}
detail_data.directions.forEach(direction => {
  const direction_li = document.createElement('li');
  direction_li.textContent = direction;
  detail_directions_list.appendChild(direction_li);
});


const other_recipe_container = document.querySelector('#detail-section2-right-other-recipe-container');
if (!other_recipe_container) {
  throw new Error('Element with ID "detail-section2-right-other-recipe-container" not found.');
}

other_recipe_data.forEach(recipe => {
  const card = Card3(recipe);
  other_recipe_container.appendChild(card);
});

const detail_section2_right_advertisement = document.querySelector('#detail-section2-right-advertisement');
if (!detail_section2_right_advertisement) {
  throw new Error('Element with ID "detail-section2-right-advertisement" not found.');
}
detail_section2_right_advertisement.appendChild(adCard());


// Detail Section 3 - Subscribe
const detail_section3 = document.querySelector('#detail-section3');
if (!detail_section3) {
...
if (!detail_section4_bottom) {
  throw new Error('Element with ID "detail-section4-bottom" not found.');
}

dishData.forEach(dish => {
  const card = Card1(dish);
  detail_section4_bottom.appendChild(card);
});
